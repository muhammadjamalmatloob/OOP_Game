...
namespace OOPFinal.BL
{
    public class Player : Character
    {
        ...
        public void HandleJump()
        {
            if (state == State.Jumping)
            {
                character.Location = new System.Drawing.Point(character.Location.X, character.Location.Y - jumpSpeed);
                jumpSpeed -= gravity; 

                if (character.Location.Y >= InitialY)
                {
                    state = State.Normal;
                    character.Location = new System.Drawing.Point(character.Location.X, InitialY); 
